...
class HomePresenceApp(ad.ADBase):
    def initialize(self):
        ...
        self.timeout = self.args.get(
            "not_home_timeout", 30
        )  # time interval before declaring not home
        self.minimum_conf = self.args.get("minimum_confidence", 90)
        self.depart_check_time = self.args.get("depart_check_time", 30)
        self.system_timeout = self.args.get(
            "system_timeout", 60
        )  # time interval for system to report back from echo
        system_check = self.args.get(
            "system_check", 30
        )  # time in seconds app queries for system online
        self.all_users_sensors = []  # used to determine if anyone at home or not
        self.not_home_timers = {}
        self.location_timers = {}
        self.home_state_entities = (
            {}
        )  # used to store or map different confidence sensors based on location to devices
        self.system_handle = {}

        self.monitor_entity = "{}.monitor_state".format(
            self.presence_topic
        )  # used to check if the network monitor is busy
        ...
        for gateway_sensor in self.args.get("home_gateway_sensors", []):
            ...
            self.hass.listen_state(
                self.gateway_opened, gateway_sensor
            )  # when the door is either opened or closed
        ...
        time = "00:00:01"
        self.adbase.run_daily(
            self.restart_device, time
        )  # restart device at midnight on sunday
        ...
        if self.system_timeout > system_check:
            ...
            self.adbase.log(
                "Cannot setup System Check due to System Timeout being Lower than System Check in Seconds",
                level="WARNING",
            )

        self.mqtt.listen_event(
            self.presence_message, "MQTT", wildcard="{}/#".format(self.presence_topic)
        )
        self.hass.listen_event(self.hass_restarted, "plugin_restarted")
        self.adbase.run_in(self.reload_device_state, 5)  # reload systems
        self.adbase.run_in(
            self.load_known_devices, 0
        )  # load up devices for all locations
    ...
    def presence_message(self, event_name, data, kwargs):
        ...
        if (
            topic.split("/")[-1] == "status"
        ):  # meaning its a message on the presence system
            location = topic.split("/")[1].replace("_", " ").title()
            siteId = location.lower().replace(" ", "_")

            self.adbase.log(
                "The Presence System in the {} is {}".format(location, payload.title()),
                level="DEBUG",
            )

            if (
                payload.title() == "Offline"
            ):  # run timer so to clear all entities for that location
        ...
        elif topic.split("/")[-1].lower() in [
            "depart",
            "arrive",
            "known device states",
            "add static device",
            "delete static device",
        ]:  # meaning its something we not interested in
            return
        ...
        if topic.split("/")[-1] == "start":  # meaning a scan is starting
            ...
            if self.mqtt.get_state(self.monitor_entity, copy=False) != "scanning":
                ...
                self.mqtt.set_state(
                    ...
                    state="scanning",
                    attributes={
                        "scan_type": topic.split("/")[2],
                        "locations": [location],
                        location: "scanning",
                    },
                )
            else:  # meaning it was already set to "scanning" already, so just update the location
                locations_attr = self.mqtt.get_state(
                    self.monitor_entity, attribute="locations"
                )
                if (
                    location not in locations_attr
                ):  # meaning it hadn't started the scan before
                    locations_attr.append(location)
                    self.mqtt.set_state(
                        self.monitor_entity,
                        attributes={"locations": locations_attr, location: "scanning"},
                    )  # update the location in the event of different scan systems in place
            ...
            if location in locations_attr:  # meaning it had started the scan before
                ...
                if locations_attr == []:  # meaning no more locations scanning
                    ...
                    )  # set the monitor state to idle
                else:
                    self.mqtt.set_state(
                        self.monitor_entity,
                        attributes={"locations": locations_attr, location: "idle"},
                    )  # update the location in the event of different scan systems in place
            return
        ...
        if topic.split("/")[-1] == "rssi":  # meaning its for rssi
        ...
        elif isinstance(payload, dict) and payload.get("type", None) in [
            "KNOWN_MAC",
            "GENERIC_BEACON",
        ]:
            ...
            if not self.hass.entity_exists(conf_sensor):  # meaning it doesn't exist
                self.adbase.log(
                    "Creating sensor {!r} for Confidence".format(conf_sensor)
                )
                self.hass.set_state(
                    conf_sensor,
                    state=confidence,
                    attributes={
                        "friendly_name": "{} {} Confidence".format(
                            friendly_name, location
                        )
                    },
                )  # create sensor for confidence

                """create user home state sensor"""
                if not self.hass.entity_exists(
                    user_device_sensor
                    ...
                    if (
                        state == "on"
                        and self.hass.get_state(self.somebody_is_home, copy=False)
                        == "off"
                ...
                self.hass.listen_state(
                    ...
                    immediate=True,
                )  # process the change immedaitely

                if device_state not in self.home_state_entities:
                    self.home_state_entities[device_state] = list()

                if (
                    conf_sensor not in self.home_state_entities[device_state]
                ):  # not really needed, but noting wrong in being extra careful
            ...
            payload[
                "location"
            ] = location  # good when writing app to triagulate location
    ...
    def confidence_updated(self, entity, attribute, old, new, kwargs):
        ...
        if user_conf_sensors != None:
            ...
            sensor_res = [
                i for i in sensor_res if i != "unknown"
            ]  # remove unknown vales from list
            sensor_res = [
                i for i in sensor_res if i != None
            ]  # remove None values from list
            if sensor_res != [] and any(
                list(map(lambda x: int(x) >= self.minimum_conf, sensor_res))
            ):  # meaning at least one of them states is greater than the minimum so device definitely home
                if (
                    self.not_home_timers[device_state] != None
                ...
                if self.hass.get_state(self.somebody_is_home, copy=False) == "off":
                    self.update_hass_sensor(
                        self.somebody_is_home, "on"
                    )  # somebody is home

                if (
                    user_device_sensor in self.all_users_sensors
                ...
                if (
                    self.not_home_timers[device_state] == None
                    and self.hass.get_state(user_device_sensor, copy=False) != "off"
                    and int(new) == 0
                ):  # run the timer
                    self.adbase.run_in(
                        self.run_arrive_scan, 0
    ...
    def not_home_func(self, kwargs):
        ...
        if all(
            list(map(lambda x: int(x) < self.minimum_conf, sensor_res))
        ):  # still confirm for the last time
            self.update_hass_sensor(user_device_sensor, "off")

            if (
                user_device_sensor in self.all_users_sensors
            ):  # check if everyone not home
    ...
    def send_mqtt_message(self, kwargs):
        ...
        if kwargs["scan_type"] == "Depart":
            ...
            if (
                self.mqtt.get_state(self.monitor_entity) == "idle"
            ):  # meaning its not busy
                self.mqtt.mqtt_publish(
                    topic, payload
                )  # send to scan for departure of anyone
                if count <= self.args.get(
                    "depart_scans", 3
    ...
    def update_hass_sensor(self, sensor, data):
        self.adbase.log(
            "__function__: Entity_ID: {}, Data: {}".format(sensor, data), level="DEBUG"
        )
    ...
    def gateway_opened(self, entity, attribute, old, new, kwargs):
        ...
        if (
            self.hass.get_state(self.everyone_not_home, copy=False) == "on"
        ):  # meaning no one at home
            self.adbase.run_in(self.run_arrive_scan, 0)

        elif (
            self.hass.get_state(self.everyone_home, copy=False) == "on"
        ):  # meaning everyone at home
    ...
    def check_home_state(self, kwargs):
        ...
        if check_state == "is_home":
            ...
            user_res = list(
                map(
                    lambda x: self.hass.get_state(x, copy=False), self.all_users_sensors
                )
            )
            user_res = [
                i for i in user_res if i != "unknown"
            ]  # remove unknown vales from list
            user_res = [i for i in user_res if i != None]  # remove None vales from list

            if all(
                list(map(lambda x: x == "on", user_res))
            ):  # meaning every one is home
                self.update_hass_sensor(self.everyone_home, "on")

            elif any(
                list(map(lambda x: x == "on", user_res))
            ):  # meaning at least someone is home
                if self.hass.get_state(self.somebody_is_home, copy=False) == "off":
                    self.update_hass_sensor(
                        self.somebody_is_home, "on"
                    )  # somebody is home
    ...
    def reload_device_state(self, kwargs):
        topic = "{}/KNOWN DEVICE STATES".format(
            self.presence_topic
        )  # get latest states
        self.adbase.run_in(
    ...
    def run_arrive_scan(self, kwargs):
        ...
        if (
            self.mqtt.get_state(self.monitor_entity, copy=False) == "idle"
        ):  # meaning its not busy
            self.mqtt.mqtt_publish(topic, payload)  # send to scan for arrival of anyone
        else:
            ...
            if (
                self.monitor_handlers.get("Arrive Scan", None) == None
                and scan_type != "arrival"
            ):  # meaning its not listening already, and arrival not running now
    ...
    def run_depart_scan(self, kwargs):
        ...
        if self.gateway_timer != None:  # meaning a timer running aleady
            self.adbase.cancel_timer(
                self.gateway_timer
            )  # just extra check, shouldn't be needed
    ...
    def remove_known_device(self, kwargs):
        ...
        self.adbase.run_in(
            self.send_mqtt_message, 0, topic=topic, payload=device, scan_type="System"
        )
    ...
    def setup_global_sensors(self):
        if not self.hass.entity_exists(
            self.everyone_not_home
        ):  # check if the sensor exist and if not create it
            ...
            )  # send to homeassistant to create binary sensor sensor for home state
